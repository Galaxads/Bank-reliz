   ...
   {                                                        
    ...
    public void zapolneniecheta(int operachet)
    {
        for (int i = 0; i < 100; i++)
        {
          ...
          /*      Console.WriteLine($"Вы заполняете его в первые или хотите перезаполнить?.");
                string otvet = Console.ReadLine();
            switch (otvet)
            {
                case "Да":

                    otk(1, "1", 1);//пустые переменные для выполнения массива 
                    Out();
                    break;
                case "Нет":
                    Out(); break;
          ...
          */
            Out();
            Console.WriteLine($"(нажмите 1)Вы хотите положить/(2)снять/(3)обнулить/(4)выполнить перевод/(5)Перезаполнить данные .");
